module CBS
	...
	def self.book
		...
		book = UI::WebDialog.new( 'book', true, 'book',400, 500, 222, 222, true )
		path = File.join(File.dirname(__FILE__), "webtool", "book.html")	
		book.set_file path # 
		 
		book.show
		book.add_action_callback("book") { |dialog,arg|
			...
			if x1 < 2 		
				...
				ent = Sketchup.active_model.entities
				 if so != 0
					group = ent.add_group  group1s, group2s, group3s, group1, group2, group3, group4, group5	
				 else	
					group = ent.add_group  group1, group2, group3, group4, group5		
				 end
			...
			if x1 == 2 
				...
				model = Sketchup.active_model
				  ...
				  entities = group7.entities
					...
					pts = []
				pts[0] = [xx/2-(d1/2), 0.mm+5.mm , 0.mm+z1]
				pts[1] = [xx/2-(d1/2)+d1, 0.mm+5.mm, 0.mm+z1]
				pts[2] = [xx/2-(d1/2)+d1, +5.mm+yy-5.mm-r1-5.mm, 0.mm+z1]
				pts[3] = [xx/2-(d1/2), +5.mm+yy-5.mm-r1-5.mm, 0.mm+z1]
			...
			if x1 >= 3 
				...
				inx = xx-d1-d1
				  ...
				  entities = group9.entities
					...
					pts = []
				pts[0] = [inx/4-(d1/2), 0.mm+5.mm , 0.mm+z1]
				pts[1] = [inx/4-(d1/2)+d1, 0.mm+5.mm, 0.mm+z1]
				pts[2] = [inx/4-(d1/2)+d1, +5.mm+yy-5.mm-r1-5.mm, 0.mm+z1]
				pts[3] = [inx/4-(d1/2), +5.mm+yy-5.mm-r1-5.mm, 0.mm+z1]
				...
				model = Sketchup.active_model
				  ...
				  entities = group10.entities
					...
					pts = []
				pts[0] = [inx/4+d1+inx/2-(d1/2), 0.mm+5.mm , 0.mm+z1]
				pts[1] = [inx/4+d1+inx/2-(d1/2)+d1, 0.mm+5.mm, 0.mm+z1]
				pts[2] = [inx/4+d1+inx/2-(d1/2)+d1, +5.mm+yy-5.mm-r1-5.mm, 0.mm+z1]
				pts[3] = [inx/4+d1+inx/2-(d1/2), +5.mm+yy-5.mm-r1-5.mm, 0.mm+z1]

				base = entities.add_face pts
				base.pushpull zz-z1-z1
					model.commit_operation
				group10.name = pa + pa22
							
				
				ent = Sketchup.active_model.entities
				 if so != 0
					group = ent.add_group  group1s, group2s, group3s, group1, group2, group3, group4, group5, group7, group9, group10	
				 else	
					group = ent.add_group  group1, group2, group3, group4, group5, group7, group9, group10		
				 end
